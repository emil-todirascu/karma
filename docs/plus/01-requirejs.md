...
For the question _"Which files do you want to include with &lt;script&gt;
tag?"_, we need to choose all files which are _not_ loaded by Require.js.
...
For the question _"What is the location of your source and test files?"_, we choose all the
...
```javascript
define(['app', 'jquery', 'underscore', './test-helper'], function (
  App,
  $,
  _,
  testHelper
