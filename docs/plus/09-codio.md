...
_If you wish, you can change the port for the `Karma Preview` entry, but make a note of the change as you will need to include that port in the `karma.config js` file later._
...
- Either:
  ...
  or - Select `Karma Start` from the Run menu (the 2nd from the right button).
...
- Switch back into your Codio project and either:
  ...
  or - Select `Karma Run` from the Run menu (the 2nd from the right button).
