...
class Browser {
  ...
  onSocketDisconnect(reason, disconnectedSocket) {
    ...
    if (this.activeSockets.length) {
      this.log.debug(
        `Disconnected ${disconnectedSocket.id}, still have ${this.getActiveSocketsIds()}`
      )
      return
    }
    ...
    if (this.isConnected()) {
      ...
      this.log.debug(
        `Disconnected during run, waiting ${this.disconnectDelay}ms for reconnecting.`
      )
      ...
      this.pendingDisconnect = this.timer.setTimeout(() => {
        ...
        this.disconnect(
          `reconnect failed before timeout of ${this.disconnectDelay}ms (${reason})`
        )
  ...
  reconnect(newSocket, clientSaysReconnect) {
    if (!clientSaysReconnect || this.state === DISCONNECTED) {
      this.log.info(
        `Disconnected browser returned on socket ${newSocket.id} with id ${this.id}.`
      )
      this.setState(CONNECTED)
      ...
      this.log.debug(
        `Rebinding to new socket ${newSocket.id} (already have ` +
  ...
  execute() {
    this.activeSockets.forEach((socket) =>
      socket.emit('execute', this.clientConfig)
    )
  ...
  disconnect(reason) {
    this.log.warn(
      `Disconnected (${this.disconnectsCount} times) ${reason || ''}`
    )
