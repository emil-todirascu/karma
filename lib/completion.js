...
function sendCompletion(possibleWords, env) {
  const regexp = new RegExp('^' + escapeForRegExp(env.last))
  possibleWords
    .filter(
      (word) =>
        regexp.test(word) &&
        !env.words.includes(word) &&
...
async function sendCompletionFiles(env) {
  const glob = globLib.glob || globLib
  const supportsCallback = typeof glob === 'function' && glob.length >= 3
  if (supportsCallback) {
    return glob(env.last + '*', { mark: true, nocase: true }, (err, files) => {
...
function complete(env) {
  ...
  if (cmdOptions) {
    if (cmdOptions[env.prev] === CUSTOM && env.last) {
      ...
      return sendCompletion(
        cmdOptions[env.prev] || Object.keys(cmdOptions),
        env
      )
    }
  }
...
function completion() {
  ...
  fs.readFile(
    path.resolve(__dirname, '../scripts/karma-completion.sh'),
    'utf8',
    function (err, data) {
      if (err) return console.error(err)
