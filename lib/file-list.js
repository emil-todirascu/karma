...
class FileList {
        ...
        const files = found
          ...
          .map((filePath) => {
            try {
              ...
              const mtime = cachedStat && cachedStat.mtime ? cachedStat.mtime : fs.statSync(filePath).mtime
              return new File(filePath, mtime, nocache, type, isBinary)
            } catch (err) {
              log.debug(`Skipping file "${filePath}" due to stat error: ${err.message}`)
              return null
            }
          })
          .filter(Boolean)
  ...
  _glob (pattern) {
    const mg = new Glob(pattern, { cwd: '/', follow: true, nodir: true, sync: true })
    return {
      found: mg.found || (typeof mg.walkSync === 'function' ? mg.walkSync() : []),
