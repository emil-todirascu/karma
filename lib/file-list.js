...
class FileList {
  ...
  _findFile(path, pattern) {
    ...
    return this._getFilesByPattern(pattern.pattern).find(
      (file) => file.originalPath === path
    )
  }

  _exists(path) {
    return !!this._patterns.find(
      (pattern) => mm(path, pattern.pattern) && this._findFile(path, pattern)
    )
  }
  ...
  _refresh() {
    ...
    lastCompletedRefresh = Promise.all(
      this._patterns.map(
        async ({ pattern, type, nocache, isBinary, integrity }) => {
          if (helper.isUrlAbsolute(pattern)) {
            this.buckets.set(pattern, [new Url(pattern, type, integrity)])
            return
          }

          const { found, statCache } = this._glob(pathLib.normalize(pattern))
          const files = found
            .filter((path) => {
            ...
            .map((filePath) => {
              try {
                const cachedStat = statCache && statCache[filePath]
                const mtime =
                  cachedStat && cachedStat.mtime
                    ? cachedStat.mtime
                    : fs.statSync(filePath).mtime
                return new File(filePath, mtime, nocache, type, isBinary)
              } catch (err) {
                log.debug(
                  `Skipping file "${filePath}" due to stat error: ${err.message}`
                )
                return null
              }
            })
          ...
          if (_.isEmpty(found)) {
            ...
            log.warn(
              `All files matched by "${pattern}" were excluded or matched by prior matchers.`
            )
          }
        }
      )
  ...
  _glob(pattern) {
    ...
    return {
      found:
        mg.found || (typeof mg.walkSync === 'function' ? mg.walkSync() : []),
      statCache: mg.statCache || null
    }
  }
  ...
  async changeFile(path, force) {
    ...
    if (!file) {
      log.debug(
        `Changed file "${path}" ignored. Does not match any file in the list.`
      )
      return this.files
    }
  ...
  async removeFile(path) {
    ...
    if (file) {
      ...
      log.debug(
        `Removed file "${path}" ignored. Does not match any file in the list.`
      )
    }
    return this.files
  }
