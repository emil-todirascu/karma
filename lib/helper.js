...
const Minimatch = minimatchLib.Minimatch

exports.browserFullNameToShort = (fullName) => {
  const ua = useragent(fullName)
  const osName = ua.os.name === 'macOS' ? 'Mac OS' : ua.os.name
  let osVersion = ua.os.version || '0.0.0'
  // ua-parser-js no longer reports Linux distro/arch in os.version for some UAs.
  // Preserve previous output format when CPU architecture is available.
  if (
    ua.os.name === 'Linux' &&
    osVersion === '0.0.0' &&
    ua.cpu &&
    ...
    osVersion = ua.cpu.architecture === 'amd64' ? 'x86_64' : ua.cpu.architecture
  }
  if (
    !ua.browser.name &&
    !ua.browser.version &&
  ...
  return `${ua.browser.name} ${ua.browser.version || '0.0.0'} (${osName} ${osVersion})`
}
...
exports.mmComparePatternWeights = (weight1, weight2) => {
  ...
  return weight1.length > 1
    ? exports.mmComparePatternWeights(weight1.slice(1), weight2.slice(1))
    : 0
}
...
exports.normalizeWinPath =
  process.platform === 'win32' ? replaceWinPath : _.identity

exports.mkdirIfNotExists = (directory, done) => {
  done = typeof done === 'function' ? done : () => {}

  // mkdirp@3 exposes promise-based API as `mkdirp.mkdirp(...)`.
  // Older versions export a callback-style function directly.
  const promiseMkdirp =
    mkdirp && typeof mkdirp.mkdirp === 'function' ? mkdirp.mkdirp : null
  if (promiseMkdirp) {
    return Promise.resolve(promiseMkdirp(directory)).then(
      () => done(),
      (err) => done(err)
    )
  }
  ...
  if (typeof mkdirp === 'function') {
    try {
      if (mkdirp.length >= 2) {
        return mkdirp(directory, done)
      }

      return Promise.resolve(mkdirp(directory)).then(
        () => done(),
        (err) => done(err)
      )
    } catch (err) {
  ...
  done(new TypeError('mkdirp is not available'))
}
