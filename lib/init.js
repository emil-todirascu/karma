...
function installPackage(pkgName) {
  ...
  try {
    ...
    // Missing package; we'll install it below.
  }
    ...
    function (err, stdout, stderr) {
      ...
      logQueue.push(function () {
        if (!err) {
          ...
          log.warn(
            `Failed to install "${pkgName}". It is not in the npm registry!\n  Please install it manually.`
          )
        } else if (/Error: EACCES/.test(stderr)) {
          log.warn(
            `Failed to install "${pkgName}". No permissions to write in ${options.cwd}!\n  Please install it manually.`
          )
        } else {
          log.warn(
...
function validatePattern(pattern) {
  const syncGlob = glob.globSync || glob.sync
  if (!syncGlob(pattern).length) {
...
function validateBrowser(name) {
  ...
  installPackage(
    'karma-' +
      name.toLowerCase().replace('headless', '').replace('canary', '') +
      '-launcher'
  )
}
...
exports.init = function (config) {
  ...
  const colorScheme =
    !helper.isDefined(config.colors) || config.colors
      ? COLOR_SCHEME.ON
      : COLOR_SCHEME.OFF
  ...
  sm.process(questions, function (answers) {
    ...
    console.log(
      colorScheme.success(`Config file generated at "${configFilePath}".`)
    )
  })
}
