...
function formatFiles(includedFiles, onlyServedFiles) {
  const lines = []
    ...
    .concat(
      onlyServedFiles.map(
        (file) => `{ pattern: ${quote(file)}, included: false }`
      )
    )
...
function formatPreprocessors(preprocessors) {
  const lines = Object.keys(preprocessors).map(
    (pattern) => `${quote(pattern)}: [${formatLine(preprocessors[pattern])}]`
  )
...
class JavaScriptFormatter {
  ...
  generateConfigFile(answers) {
    ...
    return FileUtils.readFile(this.TEMPLATE_FILE_PATH).replace(
      /%(.*)%/g,
      (a, key) => replacements[key]
    )
  }
