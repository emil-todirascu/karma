...
const captureTimeoutDecorator =
  require('./launchers/capture_timeout').decoratorFactory
...
class Launcher {
  ...
  launchSingle(
    ...
    return (name) => {
      ...
      try {
        browser = this._injector
          .createChild([locals], ['launcher:' + name])
          .get('launcher:' + name)
      } catch (e) {
        if (e.message.includes(`No provider for "launcher:${name}"`)) {
          log.error(
            `Cannot load browser "${name}": it is not registered! Perhaps you are missing some plugin?`
          )
        } else {
  ...
  launch(names, concurrency) {
    log.info(
      `Launching browsers ${names.join(', ')} with concurrency ${concurrency === Infinity ? 'unlimited' : concurrency}`
    )
    ...
    if (this._server.loadErrors.length) {
      ...
      names.forEach((name) =>
        this._injector.invoke(this.launchSingle, this)(name)
      )
    }
  ...
  killAll(callback) {
    ...
    Promise.all(this._browsers.map((browser) => browser.forceKill())).then(
      callback
    )
  }
  ...
  markCaptured(id) {
    ...
    if (browser) {
      ...
      log.debug(
        `${browser.name} (id ${browser.id}) captured in ${(Date.now() - this._lastStartTime) / 1000} secs`
      )
    }
  }
