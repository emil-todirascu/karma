...
function ProcessLauncher(spawn, tempDir, timer, processKillTimeout) {
  ...
  this._execCommand = function (cmd, args) {
    if (!cmd) {
      log.error(
        `No binary for ${self.name} browser on your platform.\n  Please, set "${self.ENV_CMD}" env variable.`
      )
  ...
  this._onProcessExit = function (code, signal, errorOutput) {
    ...
    log.debug(
      `Process ${self.name} exited with code ${code} and signal ${signal}`
    )
  ...
  this._onKillTimeout = function () {
    ...
    log.warn(
      `${self.name} was not killed in ${killTimeout} ms, sending SIGKILL.`
    )
    ...
    self._killTimer = timer.setTimeout(function () {
      log.warn(
        `${self.name} was not killed by SIGKILL in ${killTimeout} ms, continuing.`
      )
