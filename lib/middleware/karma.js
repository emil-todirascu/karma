...
function filePathToUrlPath(filePath, basePath, urlRoot, proxyPath) {
  if (filePath.startsWith(basePath)) {
    return (
      proxyPath + urlRoot.slice(1) + 'base' + filePath.slice(basePath.length)
    )
  }
...
function createKarmaMiddleware(
  ...
  return function (request, response, next) {
    ...
    const customClientContextFile = injector.get(
      'config.customClientContextFile'
    )
    const includeCrossOriginAttribute = injector.get(
      'config.crossOriginAttribute'
    )
        ...
        return serveStaticFile(
          ...
          (data) =>
            data
              .replace(
                '%X_UA_COMPATIBLE%',
                getXUACompatibleMetaElement(request.url)
              )
              .replace(
                '%X_UA_COMPATIBLE_URL%',
                getXUACompatibleUrl(request.url)
              )
        )
      }
    }
    ...
    const isRequestingClientContextFile =
      requestUrl === '/client_with_context.html'
    if (
      isRequestingContextFile ||
      isRequestingDebugFile ||
      isRequestingClientContextFile
    ) {
      return filesPromise.then((files) => {
        ...
        if (isRequestingContextFile && customContextFile) {
          ...
          log.debug(
            `Serving customClientContextFile ${customClientContextFile}`
          )
          fileServer = serveFile
          ...
          function (data) {
            ...
            for (const file of files.included) {
              ...
              if (!FILE_TYPES.includes(fileType)) {
                if (file.type == null) {
                  ...
                  log.warn(
                    `Invalid file type (${file.type || 'empty string'}), defaulting to js.`
                  )
                }
              }
              ...
              const integrityAttribute = file.integrity
                ? ` integrity="${file.integrity}"`
                : ''
              const crossOriginAttribute = includeCrossOriginAttribute
                ? ' crossorigin="anonymous"'
                : ''
              if (fileType === 'css') {
                scriptTags.push(
                  `<link type="text/css" href="${filePath}" rel="stylesheet"${integrityAttribute}${crossOriginAttribute}>`
                )
              } else if (fileType === 'dom') {
                scriptTags.push(file.content)
              } else if (fileType === 'html') {
                scriptTags.push(
                  `<link href="${filePath}" rel="import"${integrityAttribute}${crossOriginAttribute}>`
                )
              } else {
                const scriptType = SCRIPT_TYPE[fileType] || 'text/javascript'
                if (fileType === 'module') {
                  scriptTags.push(
                    `<script onerror="throw 'Error loading ${filePath}'" type="${scriptType}" src="${filePath}"${integrityAttribute}${crossOriginAttribute}></script>`
                  )
                } else {
                  scriptTags.push(
                    `<script type="${scriptType}" src="${filePath}"${integrityAttribute}${crossOriginAttribute}></script>`
                  )
                }
              }
            }
            ...
            const mappings = data.includes('%MAPPINGS%')
              ? files.served.map((file) => {
                  const filePath = filePathToUrlPath(
                    file.path,
                    basePath,
                    urlRoot,
                    proxyPath
            ...
            return data
              ...
              .replace(
                '%CLIENT_CONFIG%',
                'window.__karma__.config = ' + JSON.stringify(client) + ';\n'
              )
              .replace(
                '%SCRIPT_URL_ARRAY%',
                () =>
