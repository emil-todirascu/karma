...
function parseProxyConfig(proxies, config) {
  proxies = proxies || []
  return _.sortBy(
    _.map(proxies, function (proxyConfiguration, proxyPath) {
      ...
      const port =
        proxyDetails.port || defaultPorts[proxyDetails.protocol] || config.port
...
function createProxyHandler(proxies, urlRoot) {
  ...
  function createProxy(request, response, next) {
    ...
    if (proxyRecord) {
      log.debug(
        `proxying request - ${request.url} to ${proxyRecord.host}:${proxyRecord.port}`
      )
  ...
  createProxy.upgrade = function (request, socket, head) {
    ...
    if (proxyRecord) {
      log.debug(
        `upgrade proxyWebSocketRequest ${request.url} to ${proxyRecord.host}:${proxyRecord.port}`
      )
      request.url = request.url.replace(proxyRecord.path, proxyRecord.baseUrl)
