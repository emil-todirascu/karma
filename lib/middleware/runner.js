...
function createRunnerMiddleware(
  ...
  /* config.hostname */ hostname /* config.port */,
  port,
  ...
  return function (request, response, next) {
    ...
    function updateClientArgs(data) {
      ...
      if (_.isEmpty(data.args)) {
        ...
        (_.isArray(data.args) && _.isArray(config.client.args)) ||
        ...
        log.warn(
          'Replacing client.args with ',
          data.args,
          ' as their types do not match.'
        )
    ...
    async function refreshFileList(data) {
      ...
      if (helper.isArray(data.changedFiles)) {
        await Promise.all(
          data.changedFiles.map(async function (filepath) {
      ...
      if (helper.isArray(data.addedFiles)) {
        await Promise.all(
          data.addedFiles.map(async function (filepath) {
      ...
      if (helper.isArray(data.removedFiles)) {
        await Promise.all(
          data.removedFiles.map(async function (filepath) {
