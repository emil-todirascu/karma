...
function createPriorityPreprocessor(
  config = {},
  preprocessorPriority,
  basePath,
  instantiatePlugin
) {
  _.union
    .apply(_, Object.values(config))
    .forEach((name) => instantiatePlugin('preprocessor', name))
  return async function preprocess(file) {
    ...
    const preprocessors = preprocessorNames
      ...
      .reduce((preProcs, name) => {
        ...
        if (!isBinary || (p && p.handleBinaryFiles)) {
          ...
          log.warn(
            `Ignored preprocessing ${file.originalPath} because ${name} has handleBinaryFiles=false.`
          )
        }
        return preProcs
      }, [])
    ...
    await runProcessors(
      preprocessors,
      file,
      isBinary ? buffer : buffer.toString()
    )
  }
}

createPriorityPreprocessor.$inject = [
