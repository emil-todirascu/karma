...
const baseReporterDecoratorFactory =
...
function createErrorFormatter(config, emitter, SourceMapConsumer) {
  ...
  return function (input, indentation) {
    ...
    let msg = input.replace(
      URL_REGEXP,
      function (stackTracePath, prefix, path, __, ___, line, ____, column) {
        const normalizedPath = prefix === 'base/' ? `${basePath}/${path}` : path
        const file = lastServedFiles.find(
        ...
        if (file && file.sourceMap && line) {
          ...
          try {
            ...
            const original = getSourceMapConsumer(
              file.sourceMap
            ).originalPositionFor({ line, column: zeroBasedColumn, bias })
            ...
            const oneBasedOriginalColumn =
              original.column == null ? original.column : original.column + 1
            ...
            log.warn(
              `An unexpected error occurred while resolving the original position for: ${stackTracePath}`
            )
            log.warn(e)
          }
        }
...
function createReporters(names, config, emitter, injector) {
  ...
  names.forEach((name) => {
    ...
    try {
      ...
      reporters.push(
        injector
          .createChild([locals], ['reporter:' + name])
          .get('reporter:' + name)
      )
    } catch (e) {
      if (e.message.includes(`No provider for "reporter:${name}"`)) {
        log.error(
          `Can not load reporter "${name}", it is not registered!\n  Perhaps you are missing some plugin?`
        )
      } else {
    ...
    if (!names.includes(colorName)) {
      try {
        log.debug(
          `Trying to load color-version of reporter: ${name} (${colorName})`
        )
        reporters.push(
          injector
            .createChild([locals], ['reporter:' + colorName])
            .get('reporter:' + name)
        )
      } catch (e) {
