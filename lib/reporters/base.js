...
const BaseReporter = function (
  ...
  this.write = function () {
    ...
    this.adapters.forEach((adapter) => {
      ...
      if (
        !helper.isDefined(this.EXCLUSIVELY_USE_COLORS) ||
        adapter.colors === this.EXCLUSIVELY_USE_COLORS
      ) {
        return adapter(msg)
      }
    })
  ...
  this.onBrowserLog = (browser, log, type) => {
    ...
    if (browserConsoleLogOptions.level) {
      const logPriority = constants.LOG_PRIORITIES.indexOf(
        browserConsoleLogOptions.level.toUpperCase()
      )
...
BaseReporter.decoratorFactory = function (
  formatError,
  reportSlow,
  useColors,
  browserConsoleLogOptions
