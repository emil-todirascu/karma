...
class Server extends KarmaEventEmitter {
  constructor(cliOptionsOrConfig, done) {
    ...
    this.log.debug(
      'Final config',
      util.inspect(config, false, /** depth **/ null)
    )

    if (!config.autoWatch && !config.singleRun) {
      this.log.warn(
        '`autowatch` and `singleRun` are both `false`. In order to execute tests use `karma run`.'
      )
    }
    ...
    this.on('load_error', (type, name) => {
      this.log.debug(
        `Registered a load error of type ${type} with name ${name}`
      )
  ...
  async start() {
    ...
    try {
      this._boundServer = await NetUtils.bindAvailablePort(
        config.port,
  ...
  async _start(
    ...
    await Promise.all(
      config.frameworks.map((framework) =>
        this._injector.get('framework:' + framework)
      )
    )
    ...
    const afterPreprocess = () => {
      ...
      webServer.listen(this._boundServer, () => {
        this.log.info(
          `Karma v${constant.VERSION} server started at ${config.protocol}//${config.hostname}:${config.port}${config.urlRoot}`
        )

        this.emit('listening', config.port)
        if (config.browsers && config.browsers.length) {
          this._injector
            .invoke(launcher.launch, launcher)
            .forEach((browserLauncher) => {
        ...
        if (this.loadErrors.length > 0) {
          this.log.error(
            new Error(
              `Found ${this.loadErrors.length} load error${this.loadErrors.length === 1 ? '' : 's'}`
            )
          )
          this._close(1)
        }
    ...
    if (
      ...
      config.browserConsoleLogOptions.path
    ) {
    ...
    socketServer.sockets.on('connection', (socket) => {
      ...
      const replySocketEvents = events.bufferEvents(socket, [
        'start',
        'info',
        'karma_error',
        'result',
      ...
      socket.on('register', (info) => {
        const knownBrowser = info.id
          ? capturedBrowsers.getById(info.id) ||
            singleRunBrowsers.getById(info.id)
          : null

        if (knownBrowser) {
          knownBrowser.reconnect(socket, info.isSocketReconnect)
        } else {
          const newBrowser = this._injector
            .createChild([
              {
                id: ['value', info.id || null],
                fullName: [
                  'value',
                  helper.isDefined(info.displayName)
                    ? info.displayName
                    : info.name
                ],
                socket: ['value', socket]
    ...
    const emitRunCompleteIfAllBrowsersDone = () => {
      if (
        Object.keys(singleRunDoneBrowsers).every(
          (key) => singleRunDoneBrowsers[key]
        )
      ) {
        this.emit(
          'run_complete',
          singleRunBrowsers,
          singleRunBrowsers.getResults(singleRunBrowserNotCaptured, config)
        )
      }
    }
    ...
    this.on('browser_complete', (completedBrowser) => {
      if (
        completedBrowser.lastResult.disconnected &&
        completedBrowser.disconnectsCount <= config.browserDisconnectTolerance
      ) {
        this.log.info(
          `Restarting ${completedBrowser.name} (${completedBrowser.disconnectsCount} of ${config.browserDisconnectTolerance} attempts)`
        )
    ...
    if (config.singleRun) {
      ...
      this.on(
        'browser_complete_with_no_more_retries',
        function (completedBrowser) {
          singleRunDoneBrowsers[completedBrowser.id] = true
    ...
    processWrapper.on('unhandledRejection', (error) => {
      this.log.error(
        `UnhandledRejection: ${error.stack || error.message || String(error)}`
      )
      reportError(error)
    })
  ...
  _detach(config, done) {
    const tmpFile = path.join(os.tmpdir(), `karma-${randomUUID()}.json`)
    this.log.info('Starting karma detached')
    this.log.info('Run "karma stop" to stop the server.')
    this.log.debug(`Writing config to tmp-file ${tmpFile}`)
    ...
    const child = spawn(
      process.argv[0],
      [path.resolve(__dirname, '../lib/detached.js'), tmpFile],
      {
        detached: true,
        stdio: 'ignore'
      }
  ...
  _close(exitCode) {
    ...
    this.emitExitAsync(exitCode)
      .catch((err) => {
        this.log.error(
          'Error while calling exit event listeners\n' + err.stack || err
        )
        return 1
      })
      .then((code) => {
...
Server.prototype._start.$inject = [
  'config',
  'launcher',
  'preprocess',
  'fileList',
