...
const stripIgnorableLauncherNoise = (output) => {
  if (!output.includes('/sys/devices/system/cpu/cpu0/cpufreq/')) {
    return output
  }
...
Given(
  'a proxy on port {int} that prepends {string} to the base path',
  async function (proxyPort, proxyPath) {
...
When(
  'I start a server in background with additional arguments: {string}',
  async function (args) {
...
When(
  'I {command} Karma with additional arguments: {string}',
...
Then(
  /^it passes with(:? (like|regexp))?:$/,
  { timeout: 10 * 1000 },
  function (mode, expectedOutput, callback) {
    const like = mode === 'like'
    const regexp = mode === 'regexp'
    const actualOutput = stripIgnorableLauncherNoise(this.lastRun.stdout)
...
Then(
  /^the (stdout|stderr) (is exactly|contains|matches RegExp):$/,
  function (outputType, comparison, expectedOutput) {
    const normalizeOutput = (text) =>
      text.trim().replace(/(Usage:|Installation:)\n[ \t]+karma/g, '$1\nkarma')

    const actualOutput = normalizeOutput(
      outputType === 'stdout' ? this.lastRun.stdout : this.lastRun.stderr
    )
...
Then(
  /^The (server|stopper) is dead(:? with exit code (\d+))?$/,
  function (stopperOrServer, code, callback) {
    const server = stopperOrServer === 'server'
    setTimeout(() => {
      const actualExitCode = server
        ? this.backgroundProcess.handle.exitCode
        : this.stopperExitCode
...
Then(
  ...
  function (filePath, expectedOutput) {
...
Then(
  /^the background (stdout|stderr) (is exactly|contains|matches RegExp):$/,
  async function (outputType, comparison, expectedOutput) {
    const message =
