  ...
  describe('JavaScript', () => {
    ...
    describe('formatAnswers', () => {
      ...
      it('should format FRAMEWORKS', () => {
        const replacements = formatter.formatAnswers(
          createAnswers({ frameworks: ['jasmine', 'requirejs'] })
        )
      ...
      it('should format FILES', () => {
        ...
        replacements = formatter.formatAnswers(
          createAnswers({ files: ['*.js', 'other/file.js'] })
        )
        expect(replacements.FILES).to.equal(
          "\n      '*.js',\n      'other/file.js'"
        )
      ...
      it('should format BROWSERS', () => {
        const replacements = formatter.formatAnswers(
          createAnswers({ browsers: ['Chrome', 'Firefox'] })
        )
        expect(replacements.BROWSERS).to.equal("'Chrome', 'Firefox'")
      })

      it('should format AUTO_WATCH', () => {
        let replacements = formatter.formatAnswers(
          createAnswers({ autoWatch: true })
        )
        expect(replacements.AUTO_WATCH).to.equal('true')

        replacements = formatter.formatAnswers(
          createAnswers({ autoWatch: false })
        )
      ...
      it('should format PREPROCESSORS', () => {
        const replacements = formatter.formatAnswers(
          createAnswers({ preprocessors: { '*.coffee': ['coffee'] } })
        )
